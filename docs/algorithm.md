...
```bash
PYTHONDONTWRITEBYTECODE=1 .venv/bin/pytest -p no:cacheprovider -W error tests/test_physics.py -k randomized -x -vv
```
...
Task 9 Step 7/8 adds a reproducible slow pytest corpus for the automatic
fitter stage gate. The tests live in `tests/test_synthetic_recovery.py`, are
marked `@pytest.mark.slow`, and are excluded from default pytest collection by
the project `addopts = "-ra -m 'not slow'"`. Run them explicitly with `-m slow`.
...
Task 9 adds a released-footprint spill-angle threshold that was not separately
enumerated in design §14.3: median absolute error `≤0.02°`, p95 `≤0.05°`.
...
```bash
cd /Users/dala/Desktop/xrr-rewrite-design/xrr_fitter && .venv/bin/pytest --strict-markers tests/test_synthetic_recovery.py --collect-only -q
```

Strict collection for the slow corpus selection:

```bash
cd /Users/dala/Desktop/xrr-rewrite-design/xrr_fitter && .venv/bin/pytest --strict-markers -m slow tests/test_synthetic_recovery.py --collect-only -q
```
...
```bash
cd /Users/dala/Desktop/xrr-rewrite-design/xrr_fitter && .venv/bin/pytest --strict-markers tests/test_synthetic_recovery.py::test_slow_corpus_definitions_cover_design_contract tests/test_synthetic_recovery.py::test_recovery_metric_helpers_filter_profiles_and_measure_p95 -q
```
...
```bash
cd /Users/dala/Desktop/xrr-rewrite-design/xrr_fitter && .venv/bin/pytest -m slow tests/test_synthetic_recovery.py -v
```

Summary metric table: **待完整 slow 运行后填充**. This document intentionally
does not record synthetic recovery pass/fail metrics until the full slow gate
has actually been executed.

## Stable Service Facade

Application code crosses the GUI-free boundary only through these stable
signatures:
...
`xrr_core.validate_project()` is the source-observation facade and returns an
ordered `ProjectValidation`; schema/object-graph validation remains an
internal save/load invariant. Source declarations are never rewritten by
observation. Invalidation is a separate explicit operation.
