...
> **状态**：2026-08-08 决策与实施均已完成，见「分类总览」的状态列。本文档本身不修改任何
...
|------|------|------|----------|----------------|
| #12.2 | 暗色主题 | A（改判） | 不触及 api | **已落地** |
| B1 | 可停靠面板布局 | B | `ProjectUiState.dock_state` + 编解码 + `api.set_dock_state` | **已落地**（已签核） |
| B2 | 向导 / 专家双模式 | A（纯呈现层） | 实测确认不触及 api | **已落地** |
...
> **状态：已落地。** `theme.plot_palette` 派生绘图结构色，在 `apply_figure_font`
> （每次绘制的公共尾部）应用；`Axes.clear()` 会丢弃刻度与标题色，因此只在构造时
> 应用是不够的。Okabe-Ito 数据系列色保持不变。
...
> **状态：已落地。** 四步引导为开场界面，每步 3 个控件（预算 10）。实施前已实测
> 确认四个步骤门槛均可由现有公开 api 读出，未新增任何 api。
