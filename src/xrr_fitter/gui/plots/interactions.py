...
class PlotInteractionController(QObject):
    ...
    def _sld_grab_roughness(self, axes: object, xdata: float, ydata: float) -> tuple[str, int, object, float] | None:
        ...
        for index, _interface_nm, _sigma_nm in draggable_roughness(getattr(self._panel, "_structure", None)):
            ...
            tip_x = float(xs[-1])
            tip_y = float(ys[0])
            if interface_nm < xdata and abs(xdata - tip_x) <= 0.05 * xspan and abs(ydata - tip_y) <= 0.08 * yspan:
    ...
    def _sld_press(self, event: object) -> None:
        ...
        if navigator is not None and navigator.mode:
            return
        axes = self._sld_axes()
        if axes is None or getattr(event, "inaxes", None) is not axes:
            return
        ...
        grab = None
        if ydata is not None and isfinite(float(ydata)):
            grab = self._sld_grab_roughness(axes, xdata, float(ydata))
        self._sld_drag = grab or self._sld_grab_interface(axes, xdata) or self._sld_grab_level(xdata)
    ...
    def _sld_release(self, event: object) -> None:
        ...
        if kind == "level":
            ydata = getattr(event, "ydata", None)
            if ydata is not None and isfinite(float(ydata)):
                edited = _edit_layer_density(structure, index, origin, float(ydata))
        else:
            xdata = getattr(event, "xdata", None)
            if xdata is not None and isfinite(float(xdata)):
                if kind == "interface":
                    edited = _edit_layer_thickness(structure, index, float(xdata))
                else:
                    edited = _edit_layer_roughness(structure, index, (float(xdata) - origin) * 10.0)
        if edited is None:
        ...
        if kind == "interface":
            ...
            for candidate, _interface_nm, width_nm in draggable_roughness(getattr(self._panel, "_structure", None)):
                if candidate == index:
                    sigma_nm = width_nm
                    break
            handle.set_xdata([origin, origin + sigma_nm])
        else:
