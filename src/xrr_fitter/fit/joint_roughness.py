...
from xrr_fitter.evaluation import roughness_dynamic_uppers, values_by_name
from xrr_fitter.model.parameters import physical_to_unit, unit_to_physical
...
def _coordinate_index(problem: object, parameter_name: str) -> int:
    return next(
        ...
        if coordinate.name == parameter_name
    )
...
def _common_upper(
    ...
    return min(
        min(definition.upper, dynamic)
        for _, _, definition, dynamic in layouts
    )
...
def apply_shared_roughness(
    ...
    for global_index, variable in enumerate(problem.global_variables):
        ...
        physical = unit_to_physical(
            layouts[0][2],
            unit[global_index],
            dynamic_upper=common_upper,
        )
        for dataset_index, local_index, definition, dynamic in layouts:
            local[dataset_index][local_index] = physical_to_unit(
                definition,
                physical,
...
def initialize_shared_roughness(
    ...
    for global_index, variable in enumerate(problem.global_variables):
        ...
        layouts = _member_layouts(problem, local, variable)
        physical = float(
            median(definition.initial for _, _, definition, _ in layouts)
        )
...
def apply_consensus_roughness(
    ...
    for global_index, variable in enumerate(problem.global_variables):
        ...
        physical = float(
            median(
                physical_by_dataset[member.dataset_id][member.parameter_name]
                for member in variable.members
            )
        )
...
def _member_physical_values(
    ...
    return tuple(
        physical_maps[dataset_indices[member.dataset_id]][member.parameter_name]
        for member in variable.members
    )
...
def rebuild_candidate_roughness(
    ...
    for global_index, variable in enumerate(problem.global_variables):
        ...
        if not np.allclose(physical, physical[0], rtol=1e-10, atol=1e-12):
            raise ValueError(
                "joint candidate shared physical roughness projection mismatch"
            )
        global_unit[global_index] = _encode_common_physical(
            layouts[0][2],
