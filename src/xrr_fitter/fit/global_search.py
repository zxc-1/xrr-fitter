...
def solve_global(
    ...
    *,
    ...
    def objective(value: np.ndarray) -> float:
        ...
        if len(trace) == len(members):
            stagnation.start_generations()
        return result.objective

    def _de_callback(xk: np.ndarray, convergence: float = 0.0) -> bool:
    ...
    optimized = differential_evolution(
        ...
        # The three-generation rule owns convergence, not population energy spread.
        tol=0.0,
        atol=-1.0,
    )
    ...
    return GlobalSearchResult(
        ...
        population_energies,
        tuple(trace),
        "three_generation_stagnation" if stagnation.stopped else str(optimized.message),
        int(optimized.nfev),
    )
