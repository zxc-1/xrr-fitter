...
def test_randomized_air_fronting_stacks_match_pinned_refnx() -> None:
    ...
    for structure_index in range(500):
        ...
        media_sld = generator.uniform(0.0, 150e-6, finite_layer_count + 1) + 1j * generator.uniform(0.0, 20e-6, finite_layer_count + 1)
        ...
        assert np.all(np.isfinite(expected_parratt)), f"pinned refnx Parratt nonfinite at structure_index={structure_index}"
