...
from xrr_fitter.model.structure import LayerSpec, MaterialSpec, SlabStack, StructureSpec
...
def test_expert_powerlaw_background_adds_diffuse_tail_in_q_space() -> None:
    ...
    stack = _stack()
    q = qz_from_theta_deg(theta, MONO.wavelength_a)
    actual = instrument_reflectivity(theta, stack, MONO, background=3e-8, powerlaw_background_amplitude=1e-10, powerlaw_background_exponent=2.5)
    np.testing.assert_allclose(actual, parratt_reflectivity(q, stack) + 3e-8 + 1e-10 * q**-2.5, rtol=1e-13)


@pytest.mark.parametrize(("amplitude", "exponent", "message"), [(-1e-10, 3.0, "powerlaw_background_amplitude"), (1e-10, 0.99, "powerlaw_background_exponent"), (1e-10, 4.01, "powerlaw_background_exponent")])
def test_powerlaw_background_rejects_invalid_parameters(amplitude: float, exponent: float, message: str) -> None:
    with pytest.raises(ValueError, match=message):
        instrument_reflectivity(np.array([0.2]), _stack(), MONO, powerlaw_background_amplitude=amplitude, powerlaw_background_exponent=exponent)
...
def test_mixed_kalpha_is_ratio_weighted_sum_of_full_single_wavelength_models() -> None:
    ...
    one = instrument_reflectivity(theta, first, BeamSpec(kind="monochromatic", wavelength_a=beam.wavelength_1_a), relative_sigma=0.002)
    two = instrument_reflectivity(theta, second, BeamSpec(kind="monochromatic", wavelength_a=beam.wavelength_2_a), relative_sigma=0.002)
...
def test_mixed_kalpha_applies_scale_once_and_samples_background_at_primary_q() -> None:
    ...
    beam = BeamSpec(kind="mixed_kalpha", intensity_ratio_21=0.37)
    first, second = _stack(), _stack(29e-6 + 0.25e-6j)
    first_curve = instrument_reflectivity(theta, first, BeamSpec(kind="monochromatic", wavelength_a=beam.wavelength_1_a), relative_sigma=0.002)
    second_curve = instrument_reflectivity(theta, second, BeamSpec(kind="monochromatic", wavelength_a=beam.wavelength_2_a), relative_sigma=0.002)
...
def test_formula_material_expands_to_different_sld_for_mixed_kalpha_wavelengths() -> None:
    ...
    assert not np.array_equal(expand_structure(structure, beam.wavelength_1_a).sld_a2, expand_structure(structure, beam.wavelength_2_a).sld_a2)
...
def test_footprint_multiplies_smeared_signal_before_background() -> None:
    ...
    actual = instrument_reflectivity(theta, stack, MONO, scale=2, background=3e-8, footprint_spill_angle_deg=0.35, relative_sigma=0.002)
...
def test_theta_domain_resolution_rejects_q_domain_widths() -> None:
    ...
    with pytest.raises(ValueError, match="q-domain and theta-domain resolution cannot be combined"):
        instrument_reflectivity(theta, _stack(), MONO, resolution_domain="theta", relative_sigma=0.01, sigma_theta_deg=0.004)
...
def test_theta_domain_resolution_matches_gaussian_smear_in_theta_space() -> None:
    ...
    expected = gaussian_smear(theta, lambda query: parratt_reflectivity(qz_from_theta_deg(query, MONO.wavelength_a), stack), absolute_sigma_a_inv=0.001)
