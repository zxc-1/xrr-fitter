...
def test_bare_substrate_matches_fresnel_amplitude() -> None:
    ...
    np.testing.assert_allclose(
        parratt_reflectivity(q, SlabStack([0, 0], [0j, sld], [0])), expected, rtol=5e-12, atol=1e-13
    )
...
def test_non_air_fronting_preserves_complete_complex_sld_difference() -> None:
    ...
    expected = np.abs((q / 2 - kz) / (q / 2 + kz)) ** 2
    np.testing.assert_allclose(
        parratt_reflectivity(q, SlabStack([0, 0], [front, back], [0])), expected, rtol=5e-12, atol=1e-13
    )


def test_absorbing_layer_never_produces_nonfinite_reflectivity() -> None:
    result = parratt_reflectivity(
        np.geomspace(1e-4, 1, 800), SlabStack([0, 400, 0], [0j, 40e-6 + 15e-6j, 20e-6 + 1e-6j], [3, 5])
    )
...
def test_equal_sld_internal_layer_disappears() -> None:
    ...
    sld = 12e-6 + 0.1e-6j
    np.testing.assert_allclose(
        parratt_reflectivity(q, SlabStack([0, 73, 0], [0j, sld, sld], [0, 0])),
        parratt_reflectivity(q, SlabStack([0, 0], [0j, sld], [0])),
        rtol=1e-10,
        atol=1e-13,
    )
...
def test_single_film_returns_finite_interference_curve() -> None:
    result = parratt_reflectivity(
        np.linspace(0.005, 0.8, 500), SlabStack([0, 137, 0], [0j, 35e-6 + 0.8e-6j, 20e-6 + 0.2e-6j], [4, 6])
    )
...
def test_zero_thickness_test_stack_matches_layer_deletion() -> None:
    ...
    np.testing.assert_allclose(
        parratt_reflectivity(q, with_zero), parratt_reflectivity(q, deleted), rtol=1e-10, atol=1e-13
    )
...
def test_roughness_reduces_high_q_envelope_and_zero_restores_ideal() -> None:
    ...
    assert np.all(
        parratt_reflectivity(q, SlabStack([0, 0], ideal.sld_a2, [8])) <= parratt_reflectivity(q, ideal) * (1 + 1e-12)
    )
