...
def test_automatic_dataset_without_structure_reports_status_without_raising() -> None:
    """An imported dataset may await structure initialization during GUI refresh."""
    dataset = replace(
        dataset_project("uninitialized"),
        structure=None,
        automation=DatasetAutomation(
            import_batch_id="batch-1",
            role=AutomaticRole.UNROUTED,
    ...
    assert (
        ...
        summary.datasets[0].layers,
        summary.uniformity,
    ) == (1, "uninitialized", AutomaticStatus.PENDING, (), ())
